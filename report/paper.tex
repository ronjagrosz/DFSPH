...
\begin{onecolabstract}
    \noindent This paper discusses the technique presented by Bender et. al.~\cite{bender} for simulating incompressible fluids with an efficient and stable Divergence-Free Smoothed Particle Hydrodynamics method. The method uses two solvers to obtain a divergence-free fluid, a divergence solver and a density solver to correct the density after the divergence has been corrected. This results in a stable an fast simulation of a divergence-free fluid even though the simulation is performed on the CPU without any parallelization. 
...
An isosurface was also introduced in the middle of the scene in order to simulate collisions with arbitrary objects. This was done in an implicit representation of geometric shapes, as function $f(x,y,z)$. Given an isovalue $C = f(\vect{x})$, point $\vect{x}$ can be classified as follows:

\begin{eqnarray}
\label{eq:classifications}
\begin{array}{ll}
...
Since the quadric surface is known analytically can the differentiation be applied to the quadric directly to get the gradient of the shape in position $\vect{x}$. This gives an efficient and analytical expression with a coefficient matrix $Q$, which is used to determine the new velocity direction of a particle after colliding with the surface.

\begin{eqnarray}
\nabla f(x,y,z)
\label{eqn:quadricnormal}
...
%*No code*!!!!!
    ...
    The simulation was developed on Mac OS and Linux.
    The Linux computer is running on Debian and has four Intel(R) Core(TM) i5-2450M CPU @ 2.50GHz, one NVIDIA GeForce 610M graphics card and 6.0 GiB RAM. One of the Mac OS computer is running on macOS Sierra and has a 1.8 GHz Intel Core i5 processor with an Intel HD Graphics 400 1536 MB graphics card. 
